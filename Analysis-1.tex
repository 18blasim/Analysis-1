...
$n! = 1 \cdot 2 \cdot 3 \cdot \ldots \cdot n$ \qquad  $0! = 1! = 1$ \quad $(n+1)! = n! \cdot (n+1)$
...
Eine Zusammenfassung wohlunterschiedener Elemente zu einer Menge\\ %Meh
...
\lim\limits_{x \rightarrow 0} \frac{\sin(x)}{x} = 1 \quad \lim\limits_{x \rightarrow 0} \frac{b sin(bx)}{x} = b^2$
...
\begin{eqnarray*}
	...
	\begin{cases}
		...
		\rho = 1 \Ra  ~\sum^\infty_{n=0} a_n \text{ keine Aussage möglich, anderen Weg versuchen}
	\end{cases}
\end{eqnarray*}
\\
...
$f(x)\begin{cases}
	\text{konvergiert absolut} & \abs{x-x_0} < R \\
	\text{divergiert} & \abs{x-x_0} > R \\
	\text{keine Aussage möglich} & \abs{x-x_0} = R
	\end{cases}$\\ \\
...
\end{equation*}
\begin{equation*}
\left( \frac{x}{x+n} \right)^n = \lim\limits_{n\to\infty}\frac{1}{e^x}
\end{equation*}
